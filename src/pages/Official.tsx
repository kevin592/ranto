...
export default function Official() {
  ...
  return <Chrome page="official" locale={locale} setLocale={setLocale} t={t}>
    ...
    <section className="contact-information section-wrap"><article><Mail size={26} strokeWidth={1.4} aria-hidden="true" /><h2>{t.contact.consumerTitle}</h2><p>{t.contact.consumerBody}</p><ul>{t.contact.prepareItems.map(item => <li key={item}>{item}</li>)}</ul></article><article><BriefcaseBusiness size={26} strokeWidth={1.4} aria-hidden="true" /><h2>{t.contact.cooperationTitle}</h2><p>{t.contact.cooperationBody}</p><div className="contact-launch"><span className="availability">{t.productUI.comingSoon}</span><p>{t.contact.availability}</p></div></article></section>
  </Chrome>
}
